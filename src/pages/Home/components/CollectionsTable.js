...
const columns = [
    {
        ...
        sortable: true,
        minWidth: '250px',
        cell: row => <Flex alignItems={'center'}><Image src={row.image} w={10} h={10} rounded={'full'} mr={2}/><Link as={ReachLink} to={`/nft/${row.slug}/dashboard?code=${row.code}`} align={'left'} _hover={{color: "green.300"}}>{row.name}</Link></Flex>
    },
    {
        name: "# Tokens",
